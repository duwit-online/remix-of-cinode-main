...
import { Plus, Trash2, GripVertical, ArrowUp, ArrowDown, Play, Power, PowerOff } from "lucide-react";
...
const AdminPlaybackSources = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="glass rounded-2xl p-4 border border-primary/30 space-y-3">
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded-xl text-sm text-muted-foreground">Cancel</button>
            <button onClick={addSource} className="px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-medium">Add</button>
          </div>
        </div>
      )}
      ...
      {testUrl && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={() => setTestUrl(null)}>
          <div className="w-full max-w-2xl aspect-video"><iframe src={testUrl} className="w-full h-full rounded-xl" allowFullScreen /></div>
        </div>
      )}
    </div>
  );
